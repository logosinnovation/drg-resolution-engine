...
// ════════════════════════════════════════════════════════════════
// CLINICAL CRITERIA DATABASE (tracer bullet — Family #1)
// Will be loaded from clinical_criteria.json when all 45 families are built
// ════════════════════════════════════════════════════════════════

const CLINICAL_FAMILIES = [
  {
    id: "resp_failure", name: "Respiratory Failure", icd10_chapter: "J",
    codes: ["J9600","J9601","J9602","J9610","J9611","J9612","J9620","J9621","J9622","J9690","J9691","J9692"],
    code_ranges: "J96.0x, J96.1x, J96.2x, J96.9x", code_count: 12,
    clinical_criteria: [
      { id: "rf_abg_hypoxic", category: "laboratory", data_type: "ABG", criterion: "PaO2 < 60 mmHg",
        detail: "Arterial blood gas showing severe hypoxemia on room air.",
        threshold: { metric: "PaO2", operator: "<", value: 60, unit: "mmHg" },
        source: "CODING_CLINIC", source_detail: "Vol. 7 No. 3, Third Quarter 1990, p.14", required: false, evidence_weight: "strong" },
      { id: "rf_spo2_hypoxic", category: "vitals", data_type: "Pulse Oximetry", criterion: "SpO2 < 90% on room air",
        detail: "Persistent oxygen saturation below 90% requiring supplemental oxygen. COPD baseline may be lower.",
        threshold: { metric: "SpO2", operator: "<", value: 90, unit: "%" },
        source: "CODING_CLINIC", source_detail: "Q4 2017, p.23", required: false, evidence_weight: "moderate" },
      { id: "rf_pf_ratio", category: "laboratory", data_type: "Calculated", criterion: "PaO2/FiO2 ratio < 300",
        detail: "P/F ratio indicating acute respiratory failure or ARDS.",
        threshold: { metric: "P/F Ratio", operator: "<", value: 300, unit: "ratio" },
        source: "ARDS_BERLIN", source_detail: "Berlin Definition of ARDS", required: false, evidence_weight: "strong" },
      { id: "rf_abg_hypercapnic", category: "laboratory", data_type: "ABG", criterion: "PaCO2 > 50 mmHg with pH < 7.35",
        detail: "Acute respiratory acidosis. Elevation of PaCO2 must be accompanied by acidemia to indicate acute failure.",
        threshold: { metric: "PaCO2", operator: ">", value: 50, unit: "mmHg" },
        source: "CODING_CLINIC", source_detail: "Historical criteria baseline for Type II failure", required: false, evidence_weight: "strong" },
    ],
    documentation_requirements: [
      { id: "rf_doc_explicit",
        requirement: "Provider must explicitly document 'acute respiratory failure' or 'acute on chronic respiratory failure'.",
        insufficient_terms: ["hypoxia","hypoxemia","respiratory distress","shortness of breath","dyspnea","desaturation","respiratory insufficiency"],
        source: "CODING_CLINIC", source_detail: "Q2 2015, p.14; Q3 2012, p.20" }
    ],
    specificity_ladder: [
      { code: "J9601", description: "Acute respiratory failure with hypoxia", cc_mcc: "MCC",
        distinguishing_evidence: "ABG PaO2 < 60 or SpO2 < 90%; P/F Ratio < 300", upgrade_from: "J9600",
        upgrade_evidence: "Specify 'hypoxic' type based on low O2 levels" },
      { code: "J9602", description: "Acute respiratory failure with hypercapnia", cc_mcc: "MCC",
        distinguishing_evidence: "ABG PaCO2 > 50 with pH < 7.35", upgrade_from: "J9600",
        upgrade_evidence: "Specify 'hypercapnic' type based on high CO2 and acidosis" },
      { code: "J9600", description: "Acute respiratory failure, unspecified type", cc_mcc: "MCC",
        specificity_note: "Same MCC weight but lacks clinical specificity. Query opportunity if ABG/Vitals indicate specific type." },
      { code: "J9621", description: "Acute on chronic respiratory failure with hypoxia", cc_mcc: "MCC",
    ...
    acuity_differentiation: {
      acute: { codes: ["J9600","J9601","J9602"], evidence: "Sudden onset, identifiable precipitant, aggressive intervention required (BiPAP, intubation, high-flow O2)." },
      chronic: { codes: ["J9610","J9611","J9612"], evidence: "Longstanding condition, compensatory metabolic alkalosis on ABG, dependent on home O2, no acute worsening." },
      acute_on_chronic: { codes: ["J9620","J9621","J9622"], evidence: "Established chronic respiratory failure with acute deterioration requiring increased O2 support above baseline." },
    },
    cdi_query_templates: [
...
function ClinicalEvidence({ code, allCodes, data }) {
  ...
  if (allCodes && family.context_modifiers) {
    ...
    if (family.context_modifiers.with_copd && allNormed.some(c => c.startsWith("J44") || c.startsWith("J43")))
      activeModifiers.push({ key: "with_copd", label: "COPD on chart", text: family.context_modifiers.with_copd });
    if (family.context_modifiers.post_surgical && allNormed.some(c => c.startsWith("0") || c.startsWith("3")))
      activeModifiers.push({ key: "post_surgical", label: "Procedure present", text: family.context_modifiers.post_surgical });
    if (family.context_modifiers.with_covid19 && allNormed.some(c => c === "U071"))
      activeModifiers.push({ key: "with_covid19", label: "COVID-19 on chart", text: family.context_modifiers.with_covid19 });
  }
